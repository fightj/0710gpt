...
const API_KEY = 'sk-proj-gHx9I432vLrPtFVaOL6LT3BlbkFJHdHWhAzitqDXNCQlR53a';
...
const chatReceive = (userMsg) => {
  // 사용자가 입력한 내용은 content에 작성하여 API 서버에 전달
  const messages = [
    // 사용자가 작성한 내용에 대한 부분
    {
      role: 'user',
  ...
  .then(res => {
    ...
    // 2. 채팅창에 메세지를 등록한다.
    addChat('receive', data);

    // 3. 채팅의 연속성을 위해 이전 메세지를 oldMsg 변수에 저장
    // messages에서 system 메세지의 content 값으로 설정됨
    oldMsg = data;
  })
